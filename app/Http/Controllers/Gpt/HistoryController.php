...
use App\Http\Requests\Paraphrase\HistoryParaphraseRequest;
...
{
    public function topicParaphrase(HistoryParaphraseRequest $request)
    {
        $request->validated();
        ...
        $response = Http::withHeaders([
            ...
            'messages' => [
                [
                    'role' => 'user',
                    'content' => 'Розкажи основну теоретичну інформацію про "' . $request->topic . '" з предмету ' . config('gpt.subjects.history'),
                ]
            ],
        ...
        $createdParaphraseId = Paraphrase::create([
            'subject' => config('gpt.subjects.history'),
            'topic' => $request->topic,
